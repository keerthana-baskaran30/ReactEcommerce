...
import { deleteCart, displayCart, productDeleted, AddToCart  } from 'store/action/actions';
                    ...
                    <tbody>
                        {
                            cart && cart.map((item) => {
                                return (
                                    <tr key={item.pid}>
                                        <td><i className="btn btn-primary" onClick={() => DeleteCart(item.pid)}>X</i></td>
                                        <td>{item.pname}</td>
                                        <td><img src={ProductImages[item.pcategory]} alt = {item.pname} style={{ width: '100px', height: '80px' }} /></td>
                                        <td>{item.pprice} $</td>
                                        <td>
                                            <span className="btn btn-primary" style={{ margin: '2px' }} onClick={() => DecreaseQuantity(item.pid)}>-</span>
                                            <span className="btn btn-info">{item.pqty}</span>
                                            <span className="btn btn-primary" style={{ margin: '2px' }} onClick={() => IncreaseQuantity(item.pid)}>+</span>
                                        </td>
